#' Sparse Mean Difference Test : covid [1,8]
#'
...
#'  @examples
#'  ## run
#'  out = sp_mean(5, 25, 100, 1)
#'
#' @export
sp_mean <- function(p, n, mreplicate, covid, ndev=1, rhos=c(seq(0.01, 2, length.out=20), seq(2.2, 5, length.out=10))){
  ## parameters
  myn    = round(n)
  myp    = round(p)
  myrhos = rhos
  ...
  for (i in 1:nrhos){ # iterate over 'nrhos' values
    ...
    my.mu1 = rep(0,myp)                            # mu1
    my.mu1[sample(1:myp,ndev)] = tgtrho            #
